...
if __name__ == '__main__':
  ...
  if FLAGS.uncertainty is not None:
    uncertainty_paths = os.path.join(FLAGS.uncertainty, "scores_softmax_3dummy_0_01_final")
  ...
  if not FLAGS.ignore_safety:
    assert(len(semantics_names) == len(scans_names))
    assert(len(panoptics_names) == len(scans_names))
  ...
  vis = LaserScanVis(scan=scan,
                     scan_names=scans_names,
                     label_names=panoptics_names,
