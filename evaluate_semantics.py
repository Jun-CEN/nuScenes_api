...
if __name__ == '__main__':
  ...
  # open datapoints file
  imageset = os.path.join(FLAGS.dataset, "lidarseg", "nuscenes_infos_val.pkl")
  ...
  for info in nusc_infos:
      ...
      semantics_filename = os.path.join(nusc.dataroot, nusc.get('lidarseg', lidar_sd_token)['filename'])
      panoptics_filename = os.path.join(nusc.dataroot, nusc.get('panoptic', lidar_sd_token)['filename'])
      points_filename = os.path.join(FLAGS.dataset, lidar_path)
      scans_names.append(points_filename)
      semantics_names.append(semantics_filename)
      panoptics_names.append(panoptics_filename)
  # get label paths
  label_names = semantics_names
  # print(label_names)

  # get predictions paths
  pred_names = []
  pred_paths = os.path.join(FLAGS.predictions, "predictions_3dummy_1_001_final_latest")
  ...
  scores_names = []
  scores_paths = os.path.join(FLAGS.predictions, "scores_softmax_3dummy_1_001_final_latest")
  # populate the label names
  ...
  if FLAGS.codalab is not None:
    ...
    for i, jacc in enumerate(class_jaccard):
      if i not in ignore:
        results["iou_"+class_strings[i]] = float(jacc)
    # save to file
